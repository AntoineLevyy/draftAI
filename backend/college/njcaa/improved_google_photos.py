#!/usr/bin/env python3
"""
Improved Google Photos Finder for Claimed Players
Uses Google Custom Search API with better logging and progress tracking.
"""
...
def load_progress_file():
    ...
    progress_files = [f for f in os.listdir('.') if f.startswith('progress_photos_') and f.endswith('.json')]
    ...
    try:
        ...
        with_photos = sum(1 for p in progress_data if p.get('photo_url'))
        print(f"📸 Progress file contains {len(progress_data)} players, {with_photos} with photos")
...
def main():
    ...
    print("🎯 Improved Google Photos Finder")
    print("=" * 50)
    ...
    if progress_data:
        print("🔄 Continuing from previous progress...")
        profiles = progress_data
        # Count existing photos
        existing_photos = sum(1 for p in profiles if p.get('photo_url'))
        print(f"📸 Already have {existing_photos} photos from previous run")
    else:
