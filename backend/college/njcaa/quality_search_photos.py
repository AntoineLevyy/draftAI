#!/usr/bin/env python3
"""
Quality Search Photos Finder
Prioritizes finding the correct person's photo over finding any photo.
"""
...
class QualityGoogleImagesFinder:
    ...
    def search_player_image(self, player_name: str, school_name: str, position: str = "") -> Optional[str]:
        """Search for a player's image using quality-focused Google Custom Search API."""
        ...
        # Create high-quality search queries that prioritize accuracy
        queries = [
            # Most specific and likely to be accurate
            f'"{clean_name}" "{school_name}" soccer player headshot',
            f'"{clean_name}" "{school_name}" men soccer roster',
            f'"{clean_name}" "{school_name}" soccer team photo',
            
            # Name + school + position (very specific)
            f'"{clean_name}" "{school_name}" {position} soccer',
            f'"{clean_name}" soccer "{school_name}" {position}',
            
            # LinkedIn style searches (usually accurate)
            f'"{clean_name}" "{school_name}" linkedin',
            f'"{clean_name}" soccer player "{school_name}"',
        ]
        
        print(f"   🔍 Trying {len(queries)} high-quality search variations...")
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if 'items' in data and data['items']:
                        # Check all results for high-quality matches
                        for j, image in enumerate(data['items']):
                            ...
                            # Use strict quality check
                            quality_score = self.get_quality_score(image_url, image_title, image_snippet, clean_name, school_name)
                            
                            if quality_score >= 7:  # High quality threshold
                                print(f"   ✅ Found high-quality image (query {i+1}, result {j+1}, score {quality_score}): {image_url[:80]}...")
                                self.successful_finds += 1
                                return image_url
                            elif quality_score >= 5:  # Medium quality - log but don't use
                                print(f"   ⚠️  Found medium-quality image (score {quality_score}) but skipping for better match")
                        
                        # If no high-quality matches found, don't return anything
                        print(f"   ❌ No high-quality matches found in query {i+1}")
                else:
        ...
        print(f"   ❌ No high-quality images found after {len(queries)} search attempts")
        self.failed_finds += 1
        return None
    
    def get_quality_score(self, image_url: str, title: str, snippet: str, player_name: str, school_name: str) -> int:
        """Get a quality score for an image (0-10, higher is better)."""
        ...
        # Check if player name appears (exact matches get higher scores)
        ...
        if player_lower in title_lower or player_lower in snippet_lower:
            score += 4  # Full name match
        elif first_name in title_lower and last_name in title_lower:
            score += 3  # Both first and last name in title
        elif first_name in title_lower or last_name in title_lower:
            score += 2  # Partial name match
        elif first_name in snippet_lower or last_name in snippet_lower:
            score += 1  # Name in snippet only
...
def main():
    """Main function to test quality-focused search on a few players."""
    
    print("🎯 Quality Search Photos Finder")
    print("=" * 50)
    ...
    finder = QualityGoogleImagesFinder(api_key, search_engine_id)
    ...
    # Test on first 5 players to see quality improvement
    test_profiles = profiles[:5]
    
    print(f"🧪 Testing quality-focused search on {len(test_profiles)} players...")
    print()
    ...
    for i, profile in enumerate(test_profiles):
        ...
        # Search for photo using quality-focused Google Images
        photo_url = finder.search_player_image(player_name, school_name, position)
        
        if photo_url:
            profile['photo_url'] = photo_url
            found_photos += 1
            print(f"   ✅ High-quality photo added successfully!")
        else:
            print(f"   ❌ No high-quality photo found")
    ...
    # Save test results
    test_filename = f"quality_search_test_{timestamp}.json"
    save_progress(updated_profiles, test_filename)
    
    print(f"\n🎉 === Test Results ===")
    print(f"📊 Total profiles processed: {len(updated_profiles)}")
    print(f"📸 High-quality photos found: {found_photos}")
