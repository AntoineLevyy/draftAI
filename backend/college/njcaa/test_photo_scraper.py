#!/usr/bin/env python3
"""
Test version of the photo scraper - only processes first 5 players
"""
...
def main():
    ...
    print(f"Testing with {len(profiles)} claimed profiles...")
    ...
    with open('test_claimed_players_with_photos.json', 'w', encoding='utf-8') as f:
        json.dump(updated_profiles, f, indent=2, ensure_ascii=False)
    
    print(f"\n=== Test Results ===")
    print(f"Total profiles tested: {len(profiles)}")
    print(f"Profiles with photos: {found_photos}")
    print(f"Success rate: {(found_photos/len(profiles)*100):.1f}%")
    print(f"Saved to: test_claimed_players_with_photos.json")
