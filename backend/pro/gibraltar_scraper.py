#!/usr/bin/env python3
"""
Gibraltar Football League (Gibraltar First Tier) Player Data Scraper using Transfermarkt API
"""
...
class GibraltarAPIScraper:
    ...
    def scrape_gibraltar_players(self, include_detailed_stats: bool = True) -> list:
...
def main():
    scraper = GibraltarAPIScraper()
    data = scraper.scrape_gibraltar_players()
    scraper.save(data, "backend/pro/gibraltar_players_api.json")
    print(f"Total players in Gibraltar Football League: {len(data)}")
    print("Gibraltar Football League scraping completed!")
