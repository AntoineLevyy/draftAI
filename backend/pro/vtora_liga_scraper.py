#!/usr/bin/env python3
"""
Vtora Liga (Bulgaria 2nd Tier) Player Data Scraper using Transfermarkt API
"""
...
class VtoraLigaAPIScraper:
    ...
    def scrape_vtora_liga_players(self, include_detailed_stats: bool = True) -> List[Dict]:
        """
        Scrape comprehensive data for all players in Vtora Liga
    ...
    def save_vtora_liga_data(self, players: List[Dict], filename: str = None):
        """Save Vtora Liga player data to JSON file"""
        if filename is None:
            filename = f"vtora_liga_players_api.json"
...
def main():
    """Main function to run the Vtora Liga scraper"""
    scraper = VtoraLigaAPIScraper()
    
    # Scrape Vtora Liga data
    print("Starting Vtora Liga player data collection...")
    players = scraper.scrape_vtora_liga_players()
    
    # Save data
    scraper.save_vtora_liga_data(players)
    
    print("Vtora Liga scraping completed!")
