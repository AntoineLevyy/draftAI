#!/usr/bin/env python3
"""
Canadian Premier League Player Data Scraper using Transfermarkt API
"""
...
class CPLAPIScraper:
    ...
    def scrape_cpl_players(self, include_detailed_stats: bool = True) -> List[Dict]:
        """
        Scrape comprehensive data for all players in CPL
    ...
    def save_cpl_data(self, players: List[Dict], filename: str = None):
        """Save CPL player data to JSON file"""
        if filename is None:
            filename = f"cpl_players_api.json"
...
def main():
    """Main function to run the CPL scraper"""
    scraper = CPLAPIScraper()
    
    # Scrape CPL data
    print("Starting Canadian Premier League player data collection...")
    players = scraper.scrape_cpl_players()
    
    # Save data
    scraper.save_cpl_data(players)
    
    print("CPL scraping completed!")
