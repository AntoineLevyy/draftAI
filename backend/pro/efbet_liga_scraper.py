#!/usr/bin/env python3
"""
Efbet Liga (Bulgaria) Player Data Scraper using Transfermarkt API
"""
...
class EfbetLigaAPIScraper:
    ...
    def scrape_efbet_liga_players(self, include_detailed_stats: bool = True) -> List[Dict]:
        """
        Scrape comprehensive data for all players in Efbet Liga
    ...
    def save_efbet_liga_data(self, players: List[Dict], filename: str = None):
        """Save Efbet Liga player data to JSON file"""
        if filename is None:
            filename = f"efbet_liga_players_api.json"
...
def main():
    """Main function to run the Efbet Liga scraper"""
    scraper = EfbetLigaAPIScraper()
    
    # Scrape Efbet Liga data
    print("Starting Efbet Liga player data collection...")
    players = scraper.scrape_efbet_liga_players()
    
    # Save data
    scraper.save_efbet_liga_data(players)
    
    print("Efbet Liga scraping completed!")
